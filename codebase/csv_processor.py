...
from config import openai_config, claude_config

def process_csv(input_csv_path, organization, ai_name, download_images=False):
    start_time = time.time()
    
    base_name = os.path.splitext(os.path.basename(input_csv_path))[0]
    prompt_version = claude_config['prompt_version'] if ai_name == 'claude' else openai_config['prompt_version']
    output_csv_name = f"{base_name}_{ai_name}_{prompt_version}_{datetime.now().strftime('%Y%m%d%H%M')}.csv"
    ...
    try:
        ...
        ai_utils = import_module(f"{ai_name.lower()}_utils")
            ...
            with open(output_csv_path, 'w', newline='', encoding='utf-8') as outfile:
                ...
                for row in reader:
                    ...
                    if image_url:
                        ...
                        if ai_name == 'openai':
                            response = ai_utils.call_openai_assistant(image_url)
                        elif ai_name == 'claude':
                            response = ai_utils.call_claude_assistant(image_url)
                        else:
                            raise ValueError(f"Invalid AI name: {ai_name}")

                        if response:
                            parsed_responses = ai_utils.parse_response(response)
