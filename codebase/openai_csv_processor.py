...
def process_csv(input_csv_path, organization, download_images=False):
    ...
    output_csv_name = f"{base_name}_openai_{prompt_version}_{datetime.now().strftime('%Y%m%d%H%M')}.csv"
    ...
    try:
        ...
        openai_utils = import_module("openai_utils")
            ...
            with open(output_csv_path, 'w', newline='', encoding='utf-8') as outfile:
                ...
                for row in reader:
                    ...
                    if image_url:
                        ...
                        response = openai_utils.call_openai_assistant(image_url)
                        if response:
                            parsed_responses = openai_utils.parse_response(response)
